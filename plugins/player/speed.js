         ...
         .then(player => {
            ...
            function setDefaultRate() {
               ...
               function isMusic() {
                  ...
                  // search for a hyphen. Ex.:"Artist - Song"
                  if (user_settings.rate_default_apply_music === 'expanded' && title.includes(' - ')) return true;
                  ...
                  return [
                     ...
                     // search in title
                     || title && ['SONG', 'SOUND', 'LYRICS', 'THEME', 'AMBIENT', '🎵', '♫', /*'REMIX',*/ 'MIX', 'OFFICIAL VIDEO', 'FEAT.', 'OPENING', 'ENDING', 'COVER', 'VOCAL', 'INSTRUMENTAL', 'BASS', 'BEAT', 'LIVE RADIO', 'ALBUM', 'PLAYLIST', 'DUBSTEP', 'DANCE VER'].some(i => title.toUpperCase().includes(i))
                     // case sensitive
                     || ['MV', 'PV', 'OST', 'NCS', 'BGM', 'EDM', 'GMV', 'AMV', 'OP', 'MMD'].some(i => title.includes(i));
   ...
   options: {
      ...
      rate_default_apply_music: {
         ...
         options: [
            ...
            { label: 'skip (extended detection)', value: 'expanded' },
