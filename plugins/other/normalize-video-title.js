_plugins.push({
   ...
   _runtime: user_settings => {
      ...
      const NORMALIZE_CLASS = 'title-normalize';
      ...
      // CSS IMPLEMENTATION START
      let css = {
         'text-transform': 'lowercase',
         'display': 'block', // required to "::first-letter"
      };

      if (user_settings.show_full_video_title) css['max-height'] = 'unset';

      // lowercase all
      YDOM.injectStyle(css, `.${NORMALIZE_CLASS}`, 'important');

      // first letter uppercase
      YDOM.injectStyle({
         'text-transform': 'uppercase',
         // color: '#8A2BE2', // indicator
      }, `.${NORMALIZE_CLASS}::first-letter`, 'important');
      // CSS IMPLEMENTATION END

      YDOM.waitHTMLElement(`${VIDEO_TITLE_SELECTOR}:not([${NORMALIZE_CLASS}])`, a => {
         // console.log('NORMALIZE_CLASS:', a);
         a.setAttribute(NORMALIZE_CLASS, true);

         let upperCaseArr = a.textContent.match(UpperCaseLetter_regex);
         // Upper case more than two words
         if (upperCaseArr && upperCaseArr.length > 2) {
            // console.log('NORMALIZE_CLASS:', a.title, upperCaseArr);
            a.classList.add(NORMALIZE_CLASS);
         }
