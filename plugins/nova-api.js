...
const NOVA = {
   ...
   */
      ...
      function getFromDescriptionText() {
         ...
         [
            ...
            ...[...document.body.querySelectorAll(`#comments #comment #comment-content ${selectorTimestampLink} + *:last-child`)]
               // .map(el => el.closest('#comment')?.innerText),
               .map(el => ({
                  'source': 'comment',
                  'text': el.closest('#comment')?.innerText,
               })),
         ]
            .forEach(data => {
   ...
   //                sec: sec,
   //                time: link.innerText,
   //                title: link.parentElement.innerText
   //                   .split('\n')
   //                   .find(line => line.includes(link.innerText))
   ...
   */
   async getChannelId(api_key) {
      ...
      //       case 'channel':
      //          if ((channelName_ = document.body.querySelector('#channel-handle')?.innerText)
